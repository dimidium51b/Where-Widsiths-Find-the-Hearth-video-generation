...
class PegasiBMonitor(ThreeDScene):
    ...
    def construct(self):
        ...
        # IMPORTANT: initialize to absolute start time so MIDI events (which use absolute timestamps)
        # become active correctly when the scene runs across multiple chunks.
        # This preserves global timing across chunked renders (Option A fix).
        time_tracker = ValueTracker(self.start_t)
        ...
        logger.info("Starting animation run_time=%.3f seconds (absolute: %.3f -> %.3f)", self.duration, self.start_t, self.start_t + self.duration)
        try:
            # Animate from absolute start_t -> start_t + duration so per-event absolute times line up
            self.play(time_tracker.animate.set_value(self.start_t + self.duration), run_time=self.duration, rate_func=linear)
            ...
            time_tracker.set_value(self.start_t + self.duration)
